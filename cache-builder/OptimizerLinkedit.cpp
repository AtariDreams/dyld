...
// Optimizes the local symbols for a given set of cache files
struct UnmappedLocalsOptimizer {

    UnmappedLocalsOptimizer(uint64_t numDylibs);

    std::vector<uint8_t>            unmappedSymbolsNListBuffer;
    uint32_t                        unmappedSymbolsNListCount = 0;
    std::vector<LocalSymbolInfo>    symbolInfos;
    SortedStringPool                symbolsStringPool;
};

UnmappedLocalsOptimizer* CacheBuilder::createLocalsOptimizer(uint64_t numDylibs) {
    return new UnmappedLocalsOptimizer(numDylibs);
}

void CacheBuilder::destroyLocalsOptimizer(UnmappedLocalsOptimizer* locals) {
    delete locals;
}

UnmappedLocalsOptimizer::UnmappedLocalsOptimizer(uint64_t numDylibs) {
    unmappedSymbolsNListBuffer.reserve(0x01000000);
    symbolInfos.reserve(numDylibs);
}

void CacheBuilder::emitLocalSymbols(UnmappedLocalsOptimizer *locals) {
    ...
    const uint32_t entriesCount  = (uint32_t)locals->symbolInfos.size();
    ...
    const uint32_t nlistCount    = locals->unmappedSymbolsNListCount;
    const uint32_t stringsSize   = (uint32_t)locals->symbolsStringPool.size();
    const uint32_t stringsOffset = nlistOffset + (uint32_t)locals->unmappedSymbolsNListBuffer.size();
    ...
    if ( ::vm_allocate(mach_task_self(), &localsBuffer, localsBufferSize, VM_FLAGS_ANYWHERE) == 0 ) {
        ...
        for (uint32_t i=0; i < entriesCount; ++i) {
            entries[i].dylibOffset        = locals->symbolInfos[i].dylibOffset;
            entries[i].nlistStartIndex    = locals->symbolInfos[i].nlistStartIndex;
            entries[i].nlistCount         = locals->symbolInfos[i].nlistCount;
        }
        // copy nlists
        uint8_t* newLocalsSymbolTable = (uint8_t*)(localsBuffer+nlistOffset);
        ::memcpy(newLocalsSymbolTable, locals->unmappedSymbolsNListBuffer.data(), locals->unmappedSymbolsNListBuffer.size());
        // copy string pool
        if ( _is64 ) {
            typedef Pointer64<LittleEndian> P;
            locals->symbolsStringPool.copyPoolAndUpdateOffsets<P>(((char*)infoHeader)+stringsOffset,
                                                                  (macho_nlist<P>*)newLocalsSymbolTable);
        } else {
        ...
        _localSymbolsRegion.buffer      = (uint8_t*)localsBuffer;
        _localSymbolsRegion.bufferSize  = localsBufferSize;
        _localSymbolsRegion.sizeInUse   = localsBufferSize;
    }
    else {
        _diagnostics.warning("could not allocate local symbols");
    }
}
...
public:
    ...
    void            copyExportedSymbols(uint8_t* newLinkEditContent, SortedStringPool& stringPool, uint32_t& offset, uint32_t& symbolIndex);
    void            copyImportedSymbols(uint8_t* newLinkEditContent, SortedStringPool& stringPool, uint32_t& offset, uint32_t& symbolIndex);
    void            copyLocalSymbols(uint8_t* newLinkEditContent, SortedStringPool& stringPool, uint32_t& offset, uint32_t& symbolIndex,
                                     UnmappedLocalsOptimizer *locals);
    ...
    static void optimizeLinkedit(Diagnostics& diagnostics,
                                 CacheBuilder::Region& readOnlyRegion,
                                 uint64_t nonLinkEditReadOnlySize,
                                 const void* containerBuffer,
                                 UnmappedLocalsOptimizer* localSymbolsOptimizer,
                                 DyldSharedCache::LocalSymbolsMode localSymbolMode,
                                 const std::vector<std::tuple<const mach_header*, const char*, DylibStripMode>>& images);
    static void mergeLinkedits(Diagnostics& diagnostics,
                               CacheBuilder::Region& readOnlyRegion,
                               uint64_t nonLinkEditReadOnlySize,
                               UnmappedLocalsOptimizer* localSymbolsOptimizer,
                               DyldSharedCache::LocalSymbolsMode localSymbolMode,
...
: _mh(mh), _containerBuffer(containerBuffer), _diagnostics(diag), _dylibID(dylibID)
{
...
template <typename P>
void LinkeditOptimizer<P>::copyLocalSymbols(uint8_t* newLinkEditContent, SortedStringPool& stringPool,
                                            uint32_t& offset, uint32_t& symbolIndex,
                                            UnmappedLocalsOptimizer *locals)
{
    LocalSymbolInfo* localInfo = nullptr;
    if ( locals != nullptr ) {
        localInfo = &locals->symbolInfos.emplace_back();
        localInfo->dylibOffset = (((uint8_t*)_mh) - (uint8_t*)_containerBuffer);
        localInfo->nlistStartIndex = locals->unmappedSymbolsNListCount;
    ...
    for (const macho_nlist<P>* entry = firstExport; entry < lastExport; ++entry) {
        ...
        *newSymbolEntry = *entry;
        if ( locals != nullptr ) {
            ...
            locals->symbolsStringPool.add(locals->unmappedSymbolsNListCount, name);
            auto it = locals->unmappedSymbolsNListBuffer.insert(locals->unmappedSymbolsNListBuffer.end(), sizeof(macho_nlist<P>), 0);
            macho_nlist<P>* unmappedSymbolEntry = (macho_nlist<P>*)&*it;
            *unmappedSymbolEntry = *entry;
            unmappedSymbolEntry->set_n_strx(0);
            ++locals->unmappedSymbolsNListCount;
        }
        else {
    ...
    if ( locals != nullptr ) {
        localInfo->nlistCount = locals->unmappedSymbolsNListCount - localInfo->nlistStartIndex;
    }
}


template <typename P>
void LinkeditOptimizer<P>::copyExportedSymbols(uint8_t* newLinkEditContent, SortedStringPool& stringPool, uint32_t& offset, uint32_t& symbolIndex)
{
...
template <typename P>
void LinkeditOptimizer<P>::copyImportedSymbols(uint8_t* newLinkEditContent, SortedStringPool& stringPool, uint32_t& offset, uint32_t& symbolIndex)
{
...
template <typename P>
void LinkeditOptimizer<P>::mergeLinkedits(Diagnostics& diagnostics,
                                          CacheBuilder::Region& readOnlyRegion,
                                          uint64_t nonLinkEditReadOnlySize,
                                          UnmappedLocalsOptimizer* localSymbolsOptimizer,
                                          DyldSharedCache::LocalSymbolsMode localSymbolMode,
                                          std::vector<LinkeditOptimizer<P>*>& optimizers)
{
    ...
    uint64_t totalUnoptLinkeditsSize = readOnlyRegion.sizeInUse - nonLinkEditReadOnlySize;
    uint8_t* newLinkEdit = (uint8_t*)calloc(totalUnoptLinkeditsSize, 1);
    SortedStringPool stringPool;
    uint32_t offset = 0;
    ...
    bool unmapLocals = ( localSymbolMode == DyldSharedCache::LocalSymbolsMode::unmap );
    ...
    for (LinkeditOptimizer<P>* op : optimizers) {
         op->copyLocalSymbols(newLinkEdit, stringPool, offset, symbolIndex,
                              unmapLocals ? localSymbolsOptimizer : nullptr);
    ...
    ::memcpy(readOnlyRegion.buffer + nonLinkEditReadOnlySize, newLinkEdit, newLinkeditAlignedSize);
    ::free(newLinkEdit);
    readOnlyRegion.sizeInUse = nonLinkEditReadOnlySize + newLinkeditAlignedSize;

    // update all load commands to new merged layout
    uint64_t linkeditsUnslidStartAddr = readOnlyRegion.unslidLoadAddress + nonLinkEditReadOnlySize;
    uint32_t linkeditsCacheFileOffset = (uint32_t)(readOnlyRegion.cacheFileOffset + nonLinkEditReadOnlySize);
    for (LinkeditOptimizer<P>* op : optimizers) {
...
template <typename P>
void LinkeditOptimizer<P>::optimizeLinkedit(Diagnostics& diagnostics,
                                            CacheBuilder::Region& readOnlyRegion,
                                            uint64_t nonLinkEditReadOnlySize,
                                            const void* containerBuffer,
                                            UnmappedLocalsOptimizer* localSymbolsOptimizer,
                                            ...
                                            const std::vector<std::tuple<const mach_header*, const char*, DylibStripMode>>& images)
{
    ...
    for (std::tuple<const mach_header*, const char*, DylibStripMode> image : images) {
        optimizers.push_back(new LinkeditOptimizer<P>(containerBuffer, (macho_header<P>*)std::get<0>(image), std::get<1>(image), diagnostics));
...
#endif
    // merge linkedit info
    mergeLinkedits(diagnostics, readOnlyRegion, nonLinkEditReadOnlySize, localSymbolsOptimizer, localSymbolMode, optimizers);
...
void CacheBuilder::optimizeLinkedit(CacheBuilder::Region& readOnlyRegion,
                                    uint64_t nonLinkEditReadOnlySize,
                                    UnmappedLocalsOptimizer* localSymbolsOptimizer,
                                    const std::vector<std::tuple<const mach_header*, const char*, DylibStripMode>>& images)
{
    ...
    if ( _is64 ) {
        return LinkeditOptimizer<Pointer64<LittleEndian>>::optimizeLinkedit(_diagnostics,
                                                                            readOnlyRegion, nonLinkEditReadOnlySize,
                                                                            buffer,
                                                                            localSymbolsOptimizer, _options.localSymbolMode,
                                                                            images);
    }
    else {
         return LinkeditOptimizer<Pointer32<LittleEndian>>::optimizeLinkedit(_diagnostics,
                                                                             readOnlyRegion, nonLinkEditReadOnlySize,
                                                                             buffer,
                                                                             localSymbolsOptimizer, _options.localSymbolMode,
                                                                             images);
